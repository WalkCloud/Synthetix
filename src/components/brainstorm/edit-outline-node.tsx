...
function EditOutlineNode({ section, path, onUpdate, onRemove, onAddChild, depth }: EditOutlineNodeProps) {
  const { t } = useLocale();
  const eo = t.brainstorm.editOutline;
  const isTop = depth === 0;
    ...
    <div className={isTop ? "rounded-[12px] border bg-card shadow-sm overflow-hidden dark:shadow-none" : undefined}>
      <div className={`flex items-center gap-2 ${isTop ? "p-3" : "py-2 pr-1"} ${indent}`}>
        ...
        <input
          ...
          placeholder={isTop ? eo.sectionTitle : eo.subSectionTitle}
        ...
        <input
          ...
          className={`shrink-0 rounded ${isTop               ? "w-16 rounded-lg border bg-muted/50 px-2 py-1 text-xs focus:ring-2 focus:ring-primary/20 dark:bg-background/35"               : "w-14 border border-border bg-card px-1.5 py-0.5 text-[11px] focus:ring-1 focus:ring-primary/20"} text-center text-muted-foreground focus:outline-none`}
          placeholder={eo.words}
        />
        <button
          onClick={() => onRemove(path)}
          className={`flex shrink-0 cursor-pointer items-center justify-center rounded text-muted-foreground hover:text-red-500 ${isTop ? "h-7 w-7 rounded-lg hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10 dark:hover:text-red-400" : "h-6 w-6"}`}
        >
          {isTop ? <Trash2 className="h-3.5 w-3.5" /> : <X className="h-3.5 w-3.5" />}
        </button>
      </div>
      {isTop && section.children && section.children.length > 0 && (
        <div className="border-t bg-muted/40 p-2 space-y-2 dark:bg-background/35">
      ...
      {(isTop || (section.children && section.children.length > 0)) && (
        <div className={`${isTop ? "pl-6 pt-1 pb-1" : "pl-4 pt-0.5 pb-0.5"}`}>
          <button onClick={() => onAddChild(path)} className="flex cursor-pointer items-center gap-1 text-[11px] font-semibold text-primary/60 hover:text-primary transition-colors">
            <Plus className="h-3 w-3" /> {eo.addSubSection}
          </button>
        </div>
      )}
      {!isTop && !section.children?.length && (
        <div className="pl-4 pt-0.5 pb-0.5">
          <button onClick={() => onAddChild(path)} className="flex cursor-pointer items-center gap-1 text-[10px] font-semibold text-primary/50 hover:text-primary transition-colors">
            <Plus className="h-2.5 w-2.5" /> {eo.addSubSection}
          </button>
        </div>
      )}
    </div>
